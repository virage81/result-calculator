...
// Note: patches haven't applied beacause file was already correct
							...
							<Card title="Калькулятор" size="default">
								<Layout.Content>
									<Row gutter={[0, 8]}>
										...
										<Col span={24}>
											<Input size={"large"} value={currentNumber} onChange={(e) => onChange(e.currentTarget.value)} />
										</Col>
										<Col span={20}>
											<Row gutter={[8, 8]}>
												{numberActions.map((n) => (
													<Col span={7} key={n}>
														<Button
															onClick={() => onChange(n)} // Use onChange instead of onAddNumber
															shape={"circle"}
															size={"large"}
														>
															{n}
